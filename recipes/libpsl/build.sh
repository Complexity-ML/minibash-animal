...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/libpsl}"
VERSION=0.21.5
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" libpsl)"
...
for tool in "$CC" "$AR" "$STRIP" "$PKG_CONFIG" meson ninja; do
  command -v "$tool" >/dev/null || { echo "libpsl: missing build tool: $tool" >&2; exit 1; }
done
...
meson setup "$WORK/build" "$WORK/source" \
  ...
  -Druntime=no -Dbuiltin=true -Ddocs=false -Dtests=false
DESTDIR="$PAYLOAD" meson install -C "$WORK/build"

"$STRIP" --strip-unneeded "$PAYLOAD"/usr/lib/libpsl.so.* 2>/dev/null || true
...

{
  ...
  echo "Build: Meson shared cross $TARGET, builtin PSL data, no IDN runtime"
  echo "Compiler: $("$CC" --version | head -1)"
} > "$PAYLOAD/usr/share/altitude/sources/libpsl.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/libpsl/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-libpsl-$VERSION-amd64.altpkg"
